...
export default function SectorsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Error saving sector');
      await fetchSectors();
      setShowForm(false);
      setEditingId(null);
      setFormData({
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-display font-bold text-3xl text-brayton-navy">Gestionar Sectores</h1>
        <button
          onClick={() => {
            setShowForm(true);
            setEditingId(null);
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-brayton-navy mb-2">Descripción Corta</label>
              <textarea
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Zap, Home, Wrench"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brayton-accent outline-none"
                />
              </div>
              <div>
      ...
      <div className="grid gap-4">
        {sectors.map((sector) => (
          <div key={sector.id} className="bg-white rounded-xl border border-gray-200 p-6 flex items-start justify-between">
            <div className="flex-1">
              <h3 className="font-bold text-lg text-brayton-navy">{sector.title}</h3>
              <p className="text-sm text-brayton-slate mt-1">{sector.description}</p>
              {sector.image_url && (
                <div className="mt-3 w-20 h-20">
                  <img src={sector.image_url} alt={sector.title} className="w-full h-full object-cover rounded" />
                </div>
              )}
            </div>
